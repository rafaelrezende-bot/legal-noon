...
export default function DashboardPage() {
  ...
  const overdue = instances.filter(
    (i) => i.status !== "concluida" && i.due_date < today
  ).length;
  const next7Days = instances.filter(
    (i) => i.status !== "concluida" && i.due_date >= today && i.due_date <= in7Days
  ).length;
    ...
    <div className="p-8">
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">Obrigações do mês</h2>
